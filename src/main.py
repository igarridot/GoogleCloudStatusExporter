import json, requests, time, os, argparse, sys
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=os.environ.get('GCP_STATUS_ENDPOINT', 'https://status.cloud.google.com/incidents.json')
    )
    parser.add_argument(
        '-p', '--listen_port',
        type=int,
...
class GCPStatusCollector(object):
  ...
  def add_metric(self, incident, metric, incident_severity, product):
        metric.add_metric([incident['id'], incident['most_recent_update']['status'], product['title'], incident['external_desc'], self.gcp_status_endpoint.removesuffix('incidents.json')+incident['uri']], incident_severity)
...
def main():
  try:
    ...
    if not args.debug_mode:
      ...
      REGISTRY.register(GCPStatusCollector(args.gcp_status_endpoint, args.listen_port, args.debug_mode, args.products, args.zones, args.manage_all_events))
