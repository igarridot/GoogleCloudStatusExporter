...
### CLI Entrypoint parameters

Parameters are passed as flags when running the binary. Example:
...
./gcp-status-exporter-linux \
  ...
  --exporter.incidents-zones "europe-west2,multizone" \
  --exporter.filtered-products "Google Cloud Datastore,Google Cloud DNS"
```

| Flag                              | Default value | Description |
|------------------------------------|:-------------:|:------------|
| --web.listen-address               | ":9118"       | Address for the web interface |
| --web.metrics-path                 | "/metrics"    | Path to expose metrics |
| --exporter.save-last-update        | false         | Save last incident update description |
| --exporter.collect-resolved-incidents | false      | Collect already resolved incidents |
| --exporter.incidents-zones         | ""            | Filter by incident zones (comma separated) |
